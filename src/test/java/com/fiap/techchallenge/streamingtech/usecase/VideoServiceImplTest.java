...
class VideoServiceTest {
    ...
    void updateVideo_Success() {
        ...
        updatedVideo.setDescription("New Description");
        updatedVideo.setUrl("https://www.updated-url.com");
        updatedVideo.setPublicationDate(LocalDate.now());
        updatedVideo.setCategories(List.of("New Category"));
    ...
    void getAllVideos_Success() {
        Video video1 = new Video("id1", "Video 1", "Descrição 1", "https://www.example.com/1", LocalDate.now(), Arrays.asList("Categoria 1", "Categoria 2"), 0L, false);
        Video video2 = new Video("id2", "Video 2", "Descrição 2", "https://www.example.com/2", LocalDate.now().minusDays(1), Arrays.asList("Categoria 1"), 0L, true);
        Video video3 = new Video("id3", "Video 3", "Descrição 3", "https://www.example.com/3", LocalDate.now().minusDays(2), Arrays.asList("Categoria 2"), 0L, false);
    ...
    void getVideosByTitleAndDate_Success() throws ParseException {
        ...
        Video video1 = new Video("id1", title, "description1", "url1", LocalDate.parse(date), Arrays.asList("category1"), 0L, false);
        Video video2 = new Video("id2", title, "description2", "url2", LocalDate.parse(date).plusDays(1), Arrays.asList("category2"), 0L, true);
    ...
    void getVideosByCategory_Success() {
        ...
        Video video1 = new Video("id1", "title1", "description1", "url1", LocalDate.now(), Arrays.asList(category), 0L, false);
        Video video2 = new Video("id2", "title2", "description2", "url2", LocalDate.now().plusDays(1), Arrays.asList(category), 0L, true);

        when(videoRepository.findVideosByCategoriesContaining(eq(category))).thenReturn(Flux.just(video1, video2));
        ...
        verify(videoRepository, times(1)).findVideosByCategoriesContaining(eq(category));
    }
